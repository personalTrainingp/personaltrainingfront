...
export const ItemPago = ({deletePay, id, montoPay, id_operador, id_forma_pago, id_tarjeta, id_tipo_tarjeta, id_banco, porcentajeBancos, cuotas=0, porcentajeBancosMasIgv}) => {
  ...
  return (
            <ul className='d-flex justify-content-between align-items-start shadow p-1 border m-0 rounded rounded-3 my-1' key={id}>
				...
				<li style={{float: 'left', width: '80%', listStyle: 'none', fontWeight: 'bold', fontSize: '25px'}}>
                    ...
                    {id_operador} | {id_forma_pago}{id_tarjeta?`/${id_tarjeta}`:''}{id_tipo_tarjeta?`/${id_tipo_tarjeta}`:''}{id_banco?`/${id_banco}`:''}
					{
						porcentajeBancos&& (
							<>
								<br/>
