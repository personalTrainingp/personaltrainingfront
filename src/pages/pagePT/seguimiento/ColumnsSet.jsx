...
const columns = [
	{
		...
		accessor: 'tb_ventum.tb_cliente.nombres_apellidos_cli',
	...
	},
	{
		...
		defaultCanSort: false,
		Cell:({row})=>{
			return `${row.original.tb_ProgramaTraining.name_pgm} ${row.original.tb_semana_training.semanas_st} SEMANAS`
		}
	},
	{
		Header: 'Vencimiento',
		accessor: 'fec_fin_mem',
		Cell: ({row})=>{
			if(row.original.tb_extension_membresia?.length>0){
				return new Date(obtenerMayorExtensionFin(row.original.tb_extension_membresia)).toLocaleDateString();
			}
			return new Date(row.original.fec_fin_mem).toLocaleDateString();
		},
		defaultCanSort: true
	},
	{
		...
		Cell: ({row})=>{
			...
			if(row.original.tb_extension_membresia?.length>0){
				const fecha_fin_nueva = new Date(obtenerMayorExtensionFin(row.original.tb_extension_membresia))
				return diasLaborables(new Date(), fecha_fin_nueva);
			}
			
			return diasLaborables(new Date(), row.original.fec_fin_mem);
		}
	},
	{
		Header: 'Estado',
		Cell: ({row})=>{
			const { diasLaborables, daysUTC } = helperFunctions()
			// console.log(JSON.stringify(row.original.tb_extension_membresia));
			// return JSON.stringify(row.original.tb_extension_membresia)
		}
	}
];
