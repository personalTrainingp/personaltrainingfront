...
dayjs.extend(utc);
export default function DataOrdenCompra({showToast, id_enterprice}) {
    locale('es')
    ...
    const { obtenerOrdenCompra, ordenCompra, obtenerProveedoresUnicos, isLoadingData } = useGf_GvStore()
    // const {dataGastos, dataProvUnicosxGasto} = useSelector(e=>e.finanzas)
    const [valueFilter, setvalueFilter] = useState([])
    useEffect(() => {
        obtenerOrdenCompra(id_enterprice)
        // obtenerProveedoresUnicos()
    }, [id_enterprice])
        useEffect(() => {
        const fetchData = () => {
            setCustomers(getCustomers(ordenCompra));
            setLoading(false);
        };
        fetchData()
        initFilters();
