...
import { DateMask, DateMaskString, FormatoDateMask, MoneyFormatter } from '@/components/CurrencyMask';
        ...
        return (
          <>
                  <span className='font-24'>
                    cantidad de programas comprados: {valueFilter?.length==0?customers?.filter(f=>f.detalleVenta_pagoVenta !== 0.0)?.length:valueFilter?.filter(f=>f.detalleVenta_pagoVenta !== 0.0)?.length}
                  </span> 
...
const horariomembresiaBodyTemplate = (rowData)=>{
  return (
    <div className={`${rowExtensionColor(rowData, 'text-primary')} fw-bold`}>
        {dayjs.utc(rowData.detalle_ventaMembresia[0]?.horario).format('hh:mm A')}
    </div>
  )
}
const pagoBodyTemplate = (rowData)=>{
  return (
    <div className={`${rowExtensionColor(rowData, 'text-primary')} fw-bold`}>
        {
        rowData.detalleVenta_pagoVenta[0]?.parametro_forma_pago?.label_param
        }
    </div>
  )
}
...
const membresiaBodyTemplate = (rowData)=>{
  return (
    <div className={`${rowExtensionColor(rowData, 'text-primary')} fw-bold`}>
        {rowData.ultimoPgm}
    </div>
    ...
    return (
        <>
          ...
          <DataTable value={customers} 
                  ...
                  globalFilterFields={["tb_cliente.nombres_apellidos_cli", "tb_empleado.nombres_apellidos_empl", "tipo_comprobante", "numero_transac", "origen"]} header={header} emptyMessage="No customers found.">
              ...
              <Column  header="Horario" body={horariomembresiaBodyTemplate} filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
