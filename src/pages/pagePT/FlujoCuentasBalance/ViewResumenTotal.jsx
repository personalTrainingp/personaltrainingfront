      ...
      return (
        <>
          <Table className="tabla-egresos fs-3" style={{ width: '100%' }} bordered>
            <thead>
              <tr>
                <td style={{width: '500px'}} className={`sticky-td-white border-top-10 border-bottom-10 border-left-10 border-right-10 bg-white`}><div className={'text-black'}>{'RESULTADO ANUAL'}</div></td>
                {
                  dataAlter.map(f=>{
                    return (
            ...
            <tbody>
                ...
                <th className={`sticky-td border-left-10 border-right-10 sticky-td-${id_enterprice}`}>UTILIDAD BRUTA</th>
              {
                dataAlter.map(e=>{
                  return (
                    <React.Fragment>
                      <td className={``}> <div className={`${e.utilidadBruta>0?'text-ISESAC':'text-change'} `} ><NumberFormatMoney amount={e.utilidadBruta}/></div> </td>
                    </React.Fragment>
                    )
                  })
                }
