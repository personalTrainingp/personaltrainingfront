...
export const TableResumen = ({dataIngresos, dataGastos, anio, id_empresa, mesesSeleccionadosNums, bgMultiValue, bgTotal, mesesNombres, totalPorMesIngresos, totalPorMesEgresos, totalPorMesIngExc}) => {
        ...
        <thead >
          <tr>
            ...
            {mesesSeleccionadosNums.map(mesNum => (
              <th key={mesNum} className={`text-white text-center p-1 fs-2 ${bgTotal}`} style={{width: '200px'}}>
                {mesesNombres[mesNum - 1]}
              </th>
            ))}
            <th className={`text-center fs-1 border-top-10 border-left-10 `} style={{width: '220px'}}>
                    {!anio?(
                        <>
                        acum. del 2024 al 2026
                        </>
                    ):(
                        <>
                        TOTAL <br/> ANUAL
            ...
            <th className={`text-center p-1 fs-1 border-top-10`} style={{width: '140px'}}>%<br/> PART. <br/> ANUAL</th>
            <th className={`text-center p-1 fs-1 border-top-10 border-right-10`} style={{width: '220px'}}>PROMEDIO<br/> MENSUAL <br/> ANUAL</th>
          </tr>
        </thead>
          <tbody>
                <tr>
                ...
                <tr>
                    ...
                    <td className="text-center fw-bolder border-right-10"  style={{fontSize: '28px'}}>
                        <div className={`bg-porsiaca text-right ${utilidad(totalIngresos, totalEgresos).espositivo?'text-ISESAC':'text-change'}`}>
                            {utilidad(totalIngresos, totalEgresos).espositivo?'+':''}<NumberFormatMoney amount={(totalIngresos-totalEgresos)/12} />
                        </div>
                    </td>
                </tr>
                <tr>
