...
export const ModalCustomMovimientosArticulo = ({show=true, onHide, id=0, movimiento, idArticulo}) => {
    ...
    const { postMovimientoArticulo, updateMovimientoArticulo } = useMovimientoArticulosStore()
    const { dataEmpresas, dataKardexMovimiento } = TerminosOnShow(show)
    const onSubmitMovimientoArticulo= ()=>{
      if(id==0){
        postMovimientoArticulo({...formState, id_articulo: idArticulo}, idArticulo, movimiento)
      }
      else{
        updateMovimientoArticulo(id, {...formState, id_articulo: idArticulo}, movimiento, idArticulo)
      }
      onCloseModal()
    }
    const onCloseModal = ()=>{
  ...
  return (
    <Dialog onHide={onCloseModal} visible={show} header={`Agregar Movimiento de ${idArticulo}`}>
        <form>
          ...
          <div className='mb-2'>
            <InputSelect label={'Lugar de destino'} nameInput={'id_motivo'} onChange={onInputChange} value={id_lugar_destino} required options={dataEmpresas}/>
          </div>
          <div className='mb-2'>
