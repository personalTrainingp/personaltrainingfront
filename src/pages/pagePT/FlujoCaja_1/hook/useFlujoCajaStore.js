...
export const useFlujoCaja = () => {
	...
	const obtenerEgresosxFecha = async (enterprice, arrayDate, tt) => {
		try {
			...
			dispatch(onSetDataTerminologiasUsadasGastos(dataParametrosGastos.termGastos));
			setdataGastosxFecha({
	...
	const obtenerIngresosxFecha = async (enterprice, arrayDate) => {
		try {
			...
			dispatch(onSetDataTerminologiasUsadasIngresos(dataParametrosGastos.termGastos));
		} catch (error) {
			console.log(error);
		}
	};
