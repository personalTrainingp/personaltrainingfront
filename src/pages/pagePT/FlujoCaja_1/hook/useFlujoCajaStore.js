...
import {
  onSetDataGrupo,
  onSetDataItems,
  onSetDataTerminologiasUsadas,
} from "@/store/dataImaginaria/imaginariaSlice";
...
export const useFlujoCaja = () => {
  ...
  const obtenerEgresosxFecha = async (enterprice, arrayDate, tt) => {
    try {
      const { data } = await PTApi.get(
        `/egreso/fecha-comprobante/${enterprice}`,
        {
          params: {
            arrayDate: [
      ...
      const dataGastosOperadoresVentas = dataPagosVentas
        .filter((item) => {
          ...
          return (
            fechaItem >= new Date(arrayDate[0]) &&
            fechaItem <= new Date(arrayDate[1])
          );
        })
        .map((m) => {
          return {
      ...
      const dataGastosEnTotal = [
        ...dataGastos,
        ...dataGastosOperadoresVentas,
      ].map((m) => {
      ...
      dispatch(onSetDataTerminologiasUsadas(dataParametrosGastos.termGastos));
      dispatch(
        onSetDataItems(aplicarTipoDeCambio(dataTipoTC, dataGastosEnTotal)),
      );
    } catch (error) {
      console.log(error);
    }
  ...
  const obtenerIngresosxFecha = async (enterprice, arrayDate) => {
    try {
      ...
      const { data: dataIngresos } = await PTApi.get(
        `/ingreso/fecha/${enterprice}`,
        {
          params: {
            arrayDate: [
              formatDateToSQLServerWithDayjs(arrayDate[0], true),
      ...
      dispatch(onSetDataItems(aplicarTipoDeCambio(dataTipoTC, totalIngresos)));
      dispatch(onSetDataTerminologiasUsadas(dataParametrosGastos.termGastos));
    } catch (error) {
      console.log(error);
    }
  ...
  const obtenerFlujoCaja = async (enterprice, arrayDate) => {
    try {
      const { data } = await PTApi.get(
        `/flujo-caja/fecha-comprobante/${enterprice}`,
        {
          params: {
            arrayDate: [
              formatDateToSQLServerWithDayjs(arrayDate[0], true),
  ...
  const obtenerGastosxFecha = async (id_empresa, arrayDate) => {
    try {
      const { data } = await PTApi.get(
        `/egreso/fecha-comprobante/${id_empresa}`,
        {
          params: {
            arrayDate: [
              formatDateToSQLServerWithDayjs(arrayDate[0], true),
