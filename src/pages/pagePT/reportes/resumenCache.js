...
export const globalCache = {
    ...
    ventasResumenPromise: null,
    ventasResumenKey: null,
...
/**
 * Fetches lightweight ventas data, deduplicating concurrent requests.
 */
export function fetchVentasResumenCached(arrayDate) {
    const startStr = formatDate(arrayDate[0], true);
    const endStr = formatDate(arrayDate[1], false);
    const cacheKey = `ventas-resumen-${startStr}-${endStr}`;

    if (globalCache.ventasResumenPromise && globalCache.ventasResumenKey === cacheKey) {
        return globalCache.ventasResumenPromise;
    }

    globalCache.ventasResumenKey = cacheKey;
    globalCache.ventasResumenPromise = PTApi.get('/reporte/reporte-obtener-ventas-resumen', {
        params: { arrayDate: [startStr, endStr] },
    })
        .then(res => res.data)
        .catch(err => {
            globalCache.ventasResumenPromise = null;
            globalCache.ventasResumenKey = null;
            throw err;
        });
...
export function fetchComparativoCached(arrayDate) {
    ...
    globalCache.comparativoPromise = PTApi.get(
        `/venta/reporte/obtener-comparativo-resumen-dashboard`, {
        params: {
