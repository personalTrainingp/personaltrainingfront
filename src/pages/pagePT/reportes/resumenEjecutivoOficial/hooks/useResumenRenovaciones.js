...
export const useResumenRenovaciones = (id_empresa, fechas, dataGroup, pgmNameByIdDynamic) => {
	...
	const fetchVencimientos = async () => {
		try {
			...
			const data = await fetchVencimientosCached(year, id_empresa);
			if (data.ok && Array.isArray(data.data)) {
				...
				data.data.forEach((row) => {
					...
					const vTotal =
						row['Vencimientos (Fec Fin)'] ?? row['VENCIMIENTOS POR MES'] ?? 0;
					const rTotal =
						row['Renovaciones (Pagadas)'] ?? row['RENOVACIONES DEL MES'] ?? 0;
					...
					map[monthKey] = {
						renovaciones: rTotal,
						porcentaje:
							vTotal > 0 ? `${((rTotal / vTotal) * 100).toFixed(1)}%` : '0.0%',
						vencimientos: vTotal,
						pendiente: pReal,
						acumulado: acumulado,
					};
