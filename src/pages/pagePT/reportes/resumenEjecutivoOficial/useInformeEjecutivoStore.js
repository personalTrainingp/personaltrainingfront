...
export const useInformeEjecutivoStore = () => {
  ...
  const obtenerVentas = async () => {
    try {
      ...
      setdataVentas({
        dataMembresias: sumarMontoTotal(
          agruparPorMesDiaFechaVenta(dataMembresias),
        ),
        dataProductos17: sumarMontoTotal(
          agruparPorMesDiaFechaVenta(dataProductos17),
        ),
        dataProductos18: sumarMontoTotal(
          agruparPorMesDiaFechaVenta(dataProductos18),
        ),
        dataVentasMap: sumarMontoTotal(
          agruparPorMesDiaFechaVenta(dataVentasMap),
        ),
...
const generarRelaciones = (grupos) => {
  ...
  Object.values(grupos).forEach((arr) => {
    for (let i = 0; i < arr.length; i++) {
      ...
      const siguienteVenta = siguiente
        ? getKey(siguiente.fecha_venta)
        : "SIN_SIGUIENTE";
