...
export const useMeta = (ventas, monthsData, customStartDay = 1, customEndDay = 31) => {
	return useMemo(() => {
		// 1. Filtrar Reinscripciones (ID 692 o 696)
		const reinscripciones = ventas.filter(
			(v) => Number(v.id_origen) === 693 || Number(v.id_origen) === 694
		);
		...
		reinscripciones.forEach((v) => {
			...
			const fullName =
				v.tb_empleado?.nombres_apellidos_empl || v.usu_venta_nombre || 'Sin Asignar';
			const repName = fullName.trim().split(' ')[0];

			let amount = 0;
			const details = v.detalle_ventaMembresia || v.detalle_venta_membresia || [];
			details.forEach(
				(det) => (amount += Number(det.tarifa_monto || det.monto || det.precio || 0))
			);
