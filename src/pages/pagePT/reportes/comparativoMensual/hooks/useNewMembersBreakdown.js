...
export const useNewMembersBreakdown = (ventas, monthsData) => {
    return useMemo(() => {
        // 1. Filtrar Nuevos (No es 691, ni 692, ni 696)
        const nuevos = ventas.filter(v => {
            const id = Number(v.id_origen);
            return id !== 691 && id !== 692 && id !== 696;
        });
