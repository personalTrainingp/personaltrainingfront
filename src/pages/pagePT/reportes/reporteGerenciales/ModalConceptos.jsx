...
export const ModalConceptos = ({dataProp, show, onHide, textEmpresa, background, header}) => {
  ...
  return (
    <>
    <Dialog visible={show} onHide={onHide} style={{width: '70rem'}}>
                <Table striped>
                  <thead className={`${background}`}>
                    <tr>
                      <th className="text-white p-1"><div style={{fontSize: '48px'}}>{header}</div></th>
                  ...
                  <tbody>
                    ...
                    <tr className={``}>
                            <td className={`text-center fw-bolder fs-1`}>
                              <div className={`bg-porsiaca text-left text-change`} onClick={()=>onClickModalDetalle(p.items)}>
                                {i+1}. {p.concepto}
                              </div>
                            </td>
