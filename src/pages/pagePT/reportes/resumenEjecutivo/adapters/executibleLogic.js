...
const getDetalleMembresias = (v) =>
	v?.detalle_ventaMembresia || v?.detalle_venta_membresia || v?.detalle_ventamembresia || [];
...
export function computeMetricsForMonth({
	...
	if (metaServTotalCut > 0) {
		...
		if (!byOrigin['facebook']) byOrigin['facebook'] = { label: 'FACEBOOK', total: 0, cant: 0 };
	...
	for (const r of reservasMF) {
		...
		const ok = ['completada', 'confirmada', 'pagada', 'no pagada', 'reprogramada'].some((e) =>
			estado.includes(e)
		);
		if (!ok) continue;
...
export function buildExecutiveTableData({
	...
	if (baseKey) {
		...
		monthDescriptors = fechas.map((f) => ({
			...
			anio: f.anio,
			mes: aliasMes(f.mes),
			dateObj: new Date(Number(f.anio), MESES.indexOf(aliasMes(f.mes)), 1),
		}));
	} else {
		monthDescriptors = getAvailableMonthsFromVentas(ventas);
	}
	...
	if (!baseMonth && selectedMonthKey) {
		baseMonth = perMonth.find((m) => aliasMes(m.mes) === selectedMonthKey) || null;
	}
	...
	const monthOrderForOrigin = (okey) => {
		...
		if (baseKey && baseMonth) {
			...
			const top3Items = scored.sort((a, b) => Number(b.val) - Number(a.val)).slice(0, 4);
