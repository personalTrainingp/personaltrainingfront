...
export const ResumenComparativo = () => {
    ...
    function agruparPorSexo(detalledata) {
        return arraySexo?.map(({ label, value, order }) => {
            const items = detalledata?.filter((item) => {
                // Buscamos el cliente dentro de la venta, manejando posibles variaciones de nombre
                const cliente = item.tb_ventum?.tb_cliente || item.tb_ventum?.tb_clientes || item.tb_venta?.tb_cliente;

                // Comparamos el valor (ej. 9 para Femenino)
                // Usamos == para permitir que compare string "9" con número 9
                return cliente?.sexo_cli == value;
            });
    ...
    function agruparPorDistrito(detalledata) {
        ...
        detalledata?.forEach((item) => {
            ...
            // 🛡️ Acceso seguro: Sequelize puede devolver tb_cliente o tb_clientes
            const cliente = item.tb_ventum?.tb_cliente || item.tb_ventum?.tb_clientes;

            // Si no hay ubigeo_nac, le ponemos "Sin Distrito"
            const distrito = cliente?.ubigeo_nac?.distrito || "Sin Distrito";
            ...
            grupo.tarifa_monto += Number(tarifa_monto || 0);
        });
    ...
    function agruparPorVendedores(data) {
        ...
        data?.forEach((item) => {
            // 🛡️ Acceso seguro y valores por defecto si el empleado no existe
            const empleado = item.tb_ventum?.tb_empleado || {};

            const {
                id_empl = 0,
                nombre_empl = "Vendedor no asignado",
                estado_empl = false
            } = empleado;

            // Buscar si ya existe el grupo por nombre
    ...
    function agruparPorEstCivil(detalledata) {
        return arrayEstadoCivil?.map(({ label, value, order }) => {
            const items = detalledata?.filter((item) => {
                // 🛡️ Buscamos el objeto cliente de forma segura (singular o plural)
                const cliente = item.tb_ventum?.tb_cliente || item.tb_ventum?.tb_clientes;

                // Comparamos el valor del parámetro (ej. 10 para Casado)
                // Usamos == para evitar problemas si uno es String y el otro Number
                return cliente?.estCivil_cli == value;
            });
    ...
    function agruparPorSesiones(data) {
        ...
        data?.forEach((item) => {
            // 🛡️ Acceso seguro: si no existe, usamos un objeto vacío por defecto
            const training = item.tb_semana_training || {};
            const { sesiones = 0, semanas_st = 0 } = training;

            // Si no hay sesiones definidas, lo agrupamos como "No especificado"
            const labelBusqueda = training.sesiones ? sesiones : "No especificado";

            let grupo = resultado?.find((g) => g.lbel === labelBusqueda);

            if (!grupo) {
                grupo = {
                    lbel: labelBusqueda,
                    propiedad: training.sesiones
                        ? `${semanas_st} SEMANAS (${sesiones} SESIONES)`
    ...
    const agruparPorRangoEdad = (data) => {
        ...
        // Inicializar agrupado con una opción para "No especificado"
        const agrupado = rangos.map(rango => ({
            propiedad: rango.rango_edad,
            items: [],
        }));

        // Agregamos un grupo extra para los que no tienen fecha de nacimiento
        const grupoSinDatos = { propiedad: "Edad no especificada", items: [] };

        data?.forEach(item => {
            // 🛡️ Acceso seguro: Sequelize puede devolver tb_cliente o tb_clientes
            const cliente = item.tb_ventum?.tb_cliente || item.tb_ventum?.tb_clientes;
            const fecha_nacimiento = cliente?.fecha_nacimiento;
            const fecha_venta = item.tb_ventum?.fecha_venta;

            if (!fecha_nacimiento) {
                grupoSinDatos.items.push(item);
                return;
            }

            const edad = calcularEdadReal(fecha_nacimiento, fecha_venta);

            // Buscamos el rango de forma más eficiente
            const rangoEncontrado = agrupado.find(r => {
                const config = rangos.find(rg => rg.rango_edad === r.propiedad);
                return edad >= config.min && edad <= config.max;
            });

            if (rangoEncontrado) {
                rangoEncontrado.items.push(item);
            } else {
                grupoSinDatos.items.push(item);
            }
        });

        // Solo incluimos el grupo de "sin datos" si tiene items
        const resultadoFinal = grupoSinDatos.items.length > 0
            ? [...agrupado, grupoSinDatos]
            : agrupado;

        return resultadoFinal.map(m => {
            return {
                ...m,
                // Aquí se integra con tu agruparPorSexo que ya maneja los IDs como 9
    ...
    const dataAlter = dataGroup.map((d, array) => {
        ...
        console.log({
            agrupadoPorHorarioDeVenta: agruparPorHorariosDeVenta(ventasSinCeros)
                .sort((a, b) => b.items.length - a.items.length)
        });
    ...
    const data = [
        {
            ...
            HTML: dataAlter.map((d, index) => {
                ...
                return (
                    <Col key={index} style={{ paddingBottom: '1px !important' }} xxl={4}>
                        <Card>
        ...
        },
        {
            ...
            HTML: dataAlterIdPgmCero.map((d, index) => {
                return (
                    <Col key={index} style={{ paddingBottom: '1px !important' }} xxl={12}>
                        <Card>
            ...
            HTML: dataAlter.map((d, index) => {
                return (
                    <Col key={index}>
                        <Row>
                            {
                                d.arTest.map((g, innerIndex) => {
                                    return (
                                        <Col key={innerIndex} style={{ paddingBottom: '1px !important', marginTop: '100px' }} xxl={12}>
                                            asdf
                                            <ItemCardPgm titleRecurrent={<div className='fs-1'>{g.propiedad}</div>} avatarPrograma={d.avatarPrograma} isSesion={true} arrayEstadistico={g.agrupadoPorSesiones} onOpenModalSOCIOS={onOpenModalSOCIOS} isViewSesiones={true} labelParam={'SESION'} />
                                        </Col>
                                    )
        ...
        },
        {
            ...
            HTML: dataAlterIdPgmCero.map((d, index) => {
                return (
                    <Col key={index} style={{ paddingBottom: '1px !important', marginTop: '100px' }} xxl={12}>
                        {/* <FormatTable data={d.agrupadoPorHorario}/> */}
                ...
                loading ?
                    ...
                    <Row>
                        {data.map((section, index) => (
                            <Col key={index} xxl={12} ref={sectionRefs[index].ref}>
